...
##################################################################################################################################################################
##################################################################################################################################################################
RP.sim.m14 <- function(nx1=50,nmc=250,nrp=1000,r=5,minsplit=3,maxcompete=0,maxsurrogate=0,usesurrogate=0) {
...
x2 <- runif(500)
x3 <- runif(500)
eps <- rnorm(500,sd=0.25)
y <- 64*((x1*x2*x3)^3)*((1-x1*x2*x3)^3) + eps
...
for (j in 1:nx1) {
	...
	for (k in 1:nmc) {
		...
		pred.N.m1 <- matrix(predict(tree.m1,test.grid.3D),nrow=125) #1  total2d
		for (i in 1:nrp) {
			pred.list.m1[[i]][k,] <- t(D.part%*%pred.N.m1)%*%R.3D[[i]]		
		}
		cat("nx1:  ",j,"          nmc:  ",k,"\n")
	}
	for (i in 1:nrp) {
...
save(BigListResult,file = 'Sim_Results_m14.rda')
